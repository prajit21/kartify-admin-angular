...
import {
  IWithdrawal,
  IWithdrawalModel,
} from "../../shared/interface/withdrawal.interface";
...
export class Withdrawal {
  ...
  withdrawal$: Observable<IWithdrawalModel> = inject(Store).select(
  ...
  constructor() {
    if (
      this.store.selectSnapshot((state) => state.account.roleName === "vendor")
    ) {
  ...
  onActionClicked(action: ITableClickedAction) {
    if (action.actionToPerform == "view")
      void this.PayoutModal().openModal(action.data);
  }
  ...
  approved(event: any) {
    this.store.dispatch(
      new UpdateWithdrawStatusAction(event.data.id, event.status),
    );
  }
}
