...
import {
  ITransactionsData,
  IVenderWallet,
} from "../../shared/interface/vendor-wallet.interface";
...
export class VendorWallet {
    ...
    if (
      ...
      this.form.controls["vendor_id"].valueChanges.subscribe((value) => {
        if (value) {
          ...
          this.store.dispatch(
            new GetVendorTransactionAction(this.paginateInitialData),
          );
          this.transactions();
        }
      });
  ...
  transactions() {
    this.vendorWallet$.subscribe((wallet) => {
      let transactions = wallet?.transactions?.data?.filter(
        (element: ITransactionsData) => {
