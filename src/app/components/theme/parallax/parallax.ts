...
export class Parallax {
  ...
  home_page$: Observable<IParallax> = inject(Store).select(ThemeState.homePage);

  public form: FormGroup;
  public page_data: IParallax;
  ...
  ngOnInit() {
    const home_page$ = this.store.dispatch(
      new GetHomePageAction({ slug: "parallax" }),
    );
  ...
  patchForm() {
    this.home_page$.subscribe((homePage) => {
      ...
      homePage?.content?.parallax_banner?.banners?.forEach(
        (banner: IParallaxBanner) =>
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IParallax): number[] {
    const result: number[] = [];
