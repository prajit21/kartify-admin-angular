...
export class Fashion4 {
  ...
  home_page$: Observable<IFashionFour> = inject(Store).select(
    ThemeState.homePage,
  );
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(
      new GetHomePageAction({ slug: "fashion_four" }),
    );
  ...
  patchForm() {
    this.home_page$.subscribe((homePage) => {
      ...
      this.selectedCategories =
        homePage?.content?.products_list?.categories?.category_ids || [];
        ...
        content: {
          ...
          products_list: {
            ...
            products: {
              ...
              product_ids:
                homePage?.content?.products_list?.products?.product_ids,
