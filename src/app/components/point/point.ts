...
import {
  IPoint,
  ITransactionsData,
} from "../../shared/interface/point.interface";
import {
  CreditPointAction,
  DebitPointAction,
...
export class Point {
  ...
  point$: Observable<IPoint> = inject(Store).select(
    PointState.point,
  ) as Observable<IPoint>;
  ...
  constructor() {
    ...
    this.form.controls["consumer_id"].valueChanges.subscribe((value) => {
      if (value) {
        ...
        this.store.dispatch(
          new GetUserTransactionAction(this.paginateInitialData),
        );
        this.point$.subscribe((point) => {
          let transactions = point?.transactions?.data?.filter(
