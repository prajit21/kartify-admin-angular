...
import {
  IOrderStatus,
  IOrderStatusModel,
} from "../../../shared/interface/order-status.interface";
import {
  IOrder,
  IOrderStatusActivities,
...
export class Details {
  ...
  orderStatus$: Observable<IOrderStatusModel> = inject(Store).select(
  ...
  updateOrderStatus(data: Select2UpdateEvent) {
    if (data && data?.value) {
      if (!this.init)
        void this.ShippingNote().openModal(this.order?.id, Number(data.value));
